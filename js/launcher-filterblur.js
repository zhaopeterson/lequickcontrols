...
bodyBg.backgroundColor="#fff";
 ...
 // ===================== EUI APPS TAPPED TO ADD TO THE RECENT
    ...
    $('#hardware-controlpanel').on('click', function(){
        $('.quickaccess-container').addClass('open-quickaccess');
        setStylesScroller();
        $('#font-sourcesans').on('tap', function(){
            $(this).toggleClass('selected')
            $('body').removeClass('font-roboto');
            $('#font-roboto').removeClass('selected')
        })

        $('#font-roboto').on('tap', function(){
            console.log("font-roboto is clicked");
            $('#font-sourcesans').removeClass('selected')
            $(this).toggleClass('selected');
            $('body').toggleClass('font-roboto')
        });

        $('#text-shadow').on('tap', function(){
            $(this).toggleClass('selected')
            if(!$('#text-shadow').hasClass('selected')) {
                $('body').addClass('no-textshadow')
            } else {
                $('body').removeClass('no-textshadow')
            }
        });

        $('#locale-chinese').on('tap', function(){
            $(this).toggleClass('selected')
            $('#locale-english').removeClass('selected')
            $('.lang-english').css('display', 'none')
            $('.lang-chinese').css('display', 'block')
        })
        $('#locale-english').on('tap', function(){
            $(this).toggleClass('selected')
            $('#locale-chinese').removeClass('selected')
            $('.lang-chinese').css('display', 'none')
            $('.lang-english').css('display', 'block')
        })

        $('#shadow-light').on('tap', function(){
            $(this).toggleClass('selected');
            console.log("shadow image button tapped")

            if ($('#shadow-light').hasClass('selected')) {
                $('.widget-card').removeClass('no-shadowimage');
                $('.widget-card').removeClass('dark-shadowimage')
                $('.widget-card').removeClass('medium-shadowimage');
                $('#shadow-medium').removeClass('selected');
                $('#shadow-dark').removeClass('selected');
            } else {
                $('.widget-card').addClass('no-shadowimage');
                $('.edit-widgets li').addClass('no-shadowimage')
        ...
        $('#shadow-medium').on('tap', function(){
            $(this).toggleClass('selected');
            console.log("shadow image button tapped")

            if ($('#shadow-medium').hasClass('selected')) {
                $('.widget-card').removeClass('dark-shadowimage')
                $('.widget-card').addClass('medium-shadowimage');

                $('#shadow-light').removeClass('selected');
                $('#shadow-dark').removeClass('selected');
            } else {
                $('.widget-card').removeClass('medium-shadowimage');
            }
        ...
        $('#shadow-dark').on('tap', function(){
            $(this).toggleClass('selected');
            console.log("shadow image button tapped")

            if ($('#shadow-dark').hasClass('selected')) {
                $('.widget-card').removeClass('medium-shadowimage')
                $('.widget-card').addClass('dark-shadowimage');
                $('#shadow-medium').removeClass('selected');
                $('#shadow-light').removeClass('selected')
            } else {
                $('.widget-card').removeClass('dark-shadowimage');            
        ...
        $('#flip-divider').on('tap', function(){
            $(this).toggleClass('selected');
            $('.widget-card').toggleClass('flip')
  
        })

        $('#shadow-border').on('tap', function(){
            $('#shadow-border').toggleClass('selected');
            if ($('#shadow-border').hasClass('selected')) {
            } else {
                $('.widget-card, .widget-banner, .edit-widgets li').css('box-shadow', 'rgba(0,0,0,0,0)')
            }
        })


        var xValue = 0, yValue=2, spreadValue=3,opacity=0.2
        ...
        $( "#shadow-opacity" ).slider({
            ...
            opacity = ((ui.value)*0.01).toFixed(2);
      
            $('#opacity').text(opacity);
            var newBorderShadowValue = xValue+ 'px ' +yValue+'px '+spreadValue+'px rgba(0,0,0,'+opacity +')'
            $('.widget-card, .widget-banner, .edit-widgets li').css('box-shadow', newBorderShadowValue);
            $('#shadow-border').addClass('selected')
            }
        });
        ...
        $( "#border-spread" ).slider({
            ...
            spreadValue = ((ui.value)*0.1).toFixed(2)
            $('#spread').text(spreadValue);
        ...
        $( "#cardscrim-opacity" ).slider({
            ...
            var newBg = 'rgba(0,0,0,'+scrimValue +')'
                $('.minus1-scrim').css('background-color', newBg)
            }
        });

        $( "#cards-padding" ).slider({
            range: "min",
            value: 27,
            min: 1,
            max: 27,
            ...
            paddingValue = (ui.value)
            console.log('ui.value is: ', ui.value)
            $('#card-padding').text(paddingValue);
            var paddingDiff = 27 - paddingValue;
            var imgIncrease = Math.floor(paddingDiff/3);
            console.log('imgIncrease is: ', imgIncrease)

            var newPadding = paddingValue + 'px'

                $('.widget-card').css('padding', newPadding);
                $('.favapps-icon-wrapper, .nearby-icon-wrapper, .favapps-add-wrapper').css({
                    'width': 83+imgIncrease + 'px',
                    'height': 101+imgIncrease*1.4 + 'px'
                })
            }
        });
        ...
        $('.bgstyle-thumb').on('tap', function(e){
            ...
            if (bgImg == 'url(images-eui6/bg-whitecrack.png)' || bgImg == 'url(images-eui6/bg-whiteshapes.png)') {
...
function setPageScroll() {
    ...
    pageScroll.on('scroll', function(){     
          if (this.x> -412) {
            var blurpx = Math.abs((this.x)*10/412);
            var opacityValue =  Math.abs((this.x)*.5/412) + 0.5
            //console.log('----------',this.x, '*****', blurpx, 'opacity', opacityValue);

            $('#blurfilter-container').css({
                '-webkit-filter':'blur('+(10-blurpx)+'px)',
            ...
            var colorChange = 95-Math.abs((this.x)*95/412);
            var bg = 'rgba(72,71,71,'+ homeDockAlpha +')'
    ...
    pageScroll.on('scrollStart', function(){
        // if (this.x>= -412) {
        //     $('#blurfilter-container').addClass('blur-bg')
        // } else {
        //     //$('#blurfilter-container').removeClass('blur-bg')
        // }
    })
       ...
       // console.log('--------scrollend---------', this.x)
        ...
        for(var i=0; i<totalPages; i++) {
            if(index==1 && scrollerWidth >824) {
                ...
                //$('.home-icons-wrapper').addClass('on-minus1')
        } else {
            ...
            //  })
            //$('.home-icons-wrapper').removeClass('on-minus1')
            }
        }
    })//end of scroll end
...
function getWidgetsLength() {
    ...
    totalLength = totalLength + noWidgets ;
